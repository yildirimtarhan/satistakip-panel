...
export default function UrunlerPanel() {
  ...
  const handleSubmit = async (e) => {
    ...
    const res = await fetch(url, {
      ...
      body: JSON.stringify({
        ...form,
        stok: toplamStok, // ✅ toplam varyant stok yaz
  ...
  const handleImageUpload = async (e) => {
    ...
    if (res.ok) setForm({ ...form, resimUrl: data.url });
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold text-orange-600 mb-6 text-center">📦 Ürün Yönetimi</h1>
          ...
          <div className="flex gap-2 mb-1">
            ...
            <button type="button" className="btn"
              onClick={()=>{
                ...
                setForm({
                  ...form,
                  varyantlar:[...form.varyantlar,{ad:el.value, stok:Number(st.value)||0}]
                });
                el.value=""; st.value="";
              }}>➕ Ekle</button>
          </div>
          ...
          <div className="flex flex-wrap gap-2 mt-2">
            {form.varyantlar.map((v,i)=>(
              <span key={i} className="px-2 py-1 bg-orange-200 rounded flex items-center gap-2">
                {v.ad} — <b>{v.stok} stk</b>
                <button className="text-red-600" onClick={()=>removeVariant(i)}>✖</button>
              </span>
            ))}
          </div>
        </div>
