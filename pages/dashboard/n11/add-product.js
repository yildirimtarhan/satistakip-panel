...
export default function AddProductToN11() {
  ...
  const fetchProducts = async () => {
    try {
      ...
      console.error("Ürün liste hatası:", err);
      setUrunler([]);
    } finally {
      setFetching(false);
    }
  ...
  const sendToN11 = async () => {
    if (!selected) return alert("⚠️ Lütfen bir ürün seçin");
    ...
    <div className="p-6">
      ...
      {fetching ? (
        ...
        <p className="text-red-600">
          ERP üzerinde hiç ürün bulunamadı. Lütfen önce ürün ekleyin.
        </p>
      ) : (
        <>
          <select
