...
export default function UrunSatis() {
  ...
  const emptyRow = { 
    barkod:"", productId:"", ad:"", varyant:"", adet:1, 
    fiyat:0, kdv:20, currency:"TRY" 
  };
  ...
  const effectiveRate = (cur) =>
    cur === "USD" ? (Number(manualRates.USD) || rates.USD)
    : cur === "EUR" ? (Number(manualRates.EUR) || rates.EUR)
    : 1;
  ...
  const handleSave = async () => {
    ...
    for (let r of rows) {
      ...
      const urun = urunler.find(u=>u._id === r.productId);
      ...
      if (stok < Number(r.adet)) {
        return alert(`❌ ${urun.ad} / ${r.varyant || "-"} stok yetersiz! (Stok: ${stok})`);
      }
  ...
  const netTotal = rows.reduce((s,r)=> s + rowTL(r), 0);
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <div className="p-3 bg-white rounded border flex-wrap flex items-center gap-2 text-sm">
        ...
        <input placeholder="EUR" className="border p-1 w-20" value={manualRates.EUR} onChange={e=>setManualRates({...manualRates,EUR:e.target.value})}/>
      ...
      <table className="w-full border text-sm">
        <thead className="bg-gray-100 text-xs">
          <tr>
            <th>Barkod</th><th>Ürün</th><th>Varyant</th><th>Adet</th><th>Fiyat</th><th>PB</th><th>KDV</th><th>TL</th><th></th>
          </tr>
        </thead>
              ...
              <td>
                {urun?.varyantlar?.length > 0 ? (
                  <select className="border p-1 w-full" value={r.varyant} onChange={(e)=>setRow(i,{varyant:e.target.value})}>
                    ...
                    {urun.varyantlar.map(v=> <option key={v.ad} value={v.ad}>{v.ad} ({v.stok})</option>)}
                  </select>
                ) : "—"}
              </td>

              <td><input className="border p-1 w-14 text-right" value={r.adet} onChange={(e)=>setRow(i,{adet:e.target.value})} /></td>
              <td><input className="border p-1 w-20 text-right" value={r.fiyat} onChange={(e)=>setRow(i,{fiyat:e.target.value})} /></td>

              <td>
                <select className="border p-1 w-full" value={r.currency} onChange={(e)=>setRow(i,{currency:e.target.value})}>
                  <option>TRY</option><option>USD</option><option>EUR</option>
                </select>
              </td>

              <td>
                <select className="border p-1 w-full" value={r.kdv} onChange={(e)=>setRow(i,{kdv:e.target.value})}>
                  <option value="0">%0</option><option value="1">%1</option><option value="8">%8</option><option value="10">%10</option><option value="20">%20</option>
                </select>
              </td>

              <td className="text-right font-medium">₺{rowTL(r).toLocaleString("tr-TR")}</td>
      ...
      <datalist id="urunList">{urunler.map(u=> <option key={u._id} value={u.ad} />)}</datalist>

      <div className="flex items-center gap-3 mt-2">
        <button onClick={addRow} className="bg-gray-200 px-3 py-1 rounded">+ Satır</button>
        <button onClick={handleSave} className="bg-blue-600 text-white px-4 py-2">Kaydet ✅</button>
        <span className="ml-auto font-bold text-lg">Genel: ₺{netTotal.toLocaleString("tr-TR")}</span>
      </div>
    </div>
  );
}
