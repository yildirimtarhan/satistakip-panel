/**
 * e-Arşiv UBL belgesi indirme
 * POST /EArchiveInvoice/GetUbl
 */
import { connectToDatabase } from "@/lib/mongodb";
import jwt from "jsonwebtoken";
import { earsivGetUbl } from "@/lib/taxten/taxtenClient";
...
export default async function handler(req, res) {
  ...
  if (uuidList.length === 0) return res.status(400).json({ error: "uuid veya uuids gerekli." });

  try {
    const data = await earsivGetUbl({
      company,
    ...
    console.error("[Taxten] E-Arşiv GetUbl:", err.response?.data || err.message);
