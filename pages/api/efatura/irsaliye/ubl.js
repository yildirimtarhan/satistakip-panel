/**
 * E-İrsaliye belge indirme (Taxten GetUbl)
 * POST /api/efatura/irsaliye/ubl
 */
import { connectToDatabase } from "@/lib/mongodb";
import jwt from "jsonwebtoken";
import { despatchGetUbl } from "@/lib/taxten/taxtenClient";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { uuid, uuids, type = "OUTBOUND", docType = "DESPATCH" } = req.body || {};
    ...
    const data = await despatchGetUbl({
      company,
    ...
    console.error("E-İrsaliye GetUbl:", err.response?.data || err.message);
