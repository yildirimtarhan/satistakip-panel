/**
 * Pazarama ürün temin bilgisi: ürüne şablon atama
 * POST product/upsertSellerProductCommercial
 */
import jwt from "jsonwebtoken";
import { getPazaramaCredentials } from "@/lib/getPazaramaCredentials";
import { pazaramaUpsertSellerProductCommercial } from "@/lib/marketplaces/pazaramaService";
...
export default async function handler(req, res) {
  ...
  const { code, securityDescription, commercialIds } = req.body || {};
  if (!code || !String(code).trim()) {
    return res.status(400).json({ success: false, error: "code (barkod) zorunlu." });
  }

  try {
    const data = await pazaramaUpsertSellerProductCommercial(creds, {
      code: code.trim(),
      securityDescription,
      commercialIds: Array.isArray(commercialIds) ? commercialIds : [],
    });
    ...
    console.error("[Pazarama] Ürün temin bilgisi:", err.message);
