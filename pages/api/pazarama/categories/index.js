...
import { pazaramaGetCategories } from "@/lib/marketplaces/pazaramaService";
...
export default async function handler(req, res) {
  ...
  try {
    const resp = await pazaramaGetCategories(creds);
    const raw = resp?.data ?? resp?.categories ?? resp;
    const list = Array.isArray(raw) ? raw : (raw?.data ? raw.data : []);
    return res.status(200).json({ success: true, data: list });
  } catch (err) {
    console.error("[Pazarama] Kategori hatası:", err.message);
