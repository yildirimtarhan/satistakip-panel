...
export async function n11CreateProduct(req, product, n11Override = {}) {
  ...
  // Zorunlu alanları kontrol et — override değerleri öncelikli
  ...
  const categoryId =
    n11Override.categoryId ||
    product.marketplaceSettings?.n11?.categoryId ||
    product.n11CategoryId ||
    product.categoryId;
  const shipmentTemplate = n11Override.shipmentTemplate || product.shipmentTemplate || "STANDART";
  ...
  const attributes = n11Override.attributes?.length
    ? n11Override.attributes
    : product.marketplaceSettings?.n11?.attributes?.length
    ? product.marketplaceSettings.n11.attributes
    : product.n11Attributes?.length
    ? product.n11Attributes
    : [];
    ...
    skus: [
      {
        ...
        // N11: catalogId, barcode veya images+attributes zorunlu (en az biri)
        ...(product.barcode || product.barkod
          ? { barcode: String(product.barcode || product.barkod) }
          : {}),
        ...
        ...(product.images?.length
          ? { images: product.images.slice(0, 8).map((img, i) => ({
              url: typeof img === "string" ? img : img?.url,
              order: i + 1,
            })).filter((i) => i.url) }
          : {}),
        // Marka — N11 REST'te SKU'da doğrudan brandId alanı
        ...(product.marketplaceSettings?.n11?.brandId
          ? { brandId: Number(product.marketplaceSettings.n11.brandId) }
          : product.n11BrandId
          ? { brandId: Number(product.n11BrandId) }
          : {}),
        // Kategori attribute'ları (override'dan veya DB'den)
        ...(attributes.length ? { attributes } : {}),
      },
    ],
  };

  console.log("[N11 product-create] Payload:", JSON.stringify(payload, null, 2));
  ...
  try {
    ...
    console.error("[N11 product-create] Status:", axiosErr?.response?.status);
    console.error("[N11 product-create] Body:", JSON.stringify(errBody));
    console.error("[N11 product-create] Payload sent:", JSON.stringify(payload));
...
export async function n11GetTaskStatus({ companyId, userId, taskId }) {
  ...
  // SKU seviyesinde durum kontrolü (task PROCESSED olsa bile SKU FAIL olabilir)
