...
export function createUbl(invoice, company) {
  ...
  const rawUuid = invoice.uuid && String(invoice.uuid).trim();
  const uuid = rawUuid && uuidRegex.test(rawUuid) ? rawUuid.toLowerCase() : (typeof require !== "undefined" ? require("crypto").randomUUID() : `${Date.now()}-${Math.random().toString(36).slice(2, 11)}`);
  ...
  const rawCompanyVkn = String(company.vkn ?? "").replace(/\D/g, "");
  const companyVkn = rawCompanyVkn.length >= 10 ? rawCompanyVkn.slice(0, 10) : rawCompanyVkn;
  ...
  // UBL-TR PostalAddress: PostalZone, Region, District, Country zorunlu
  const supplierPostalZone = postalZone || "00000";
  const supplierRegion = region || city || district || "Türkiye";
  const supplierDistrict = district || city || "-";
  ...
  // Müşteri bilgileri (VKN 10 hane, TCKN 11 hane – şematron uyumu)
  const rawCustomerVkn = String(customer.vknTckn || customer.vkn || customer.identifier || "").replace(/\D/g, "");
  const isCompany = rawCustomerVkn.length === 10;
  const customerVkn = isCompany ? rawCustomerVkn.slice(0, 10) : rawCustomerVkn.slice(0, 11);
  const customerTitle = customer.title || customer.unvan || customer.ad || "Müşteri";
  const customerFirstName = customer.firstName || customer.adi || "";
  const customerFamilyName = customer.familyName || customer.soyadi || (customerTitle && customerTitle.trim().split(/\s+/).length > 1 ? customerTitle.trim().split(/\s+/).slice(-1)[0] : customerTitle) || "";
  const personFirstName = customerFirstName || (customerTitle && customerTitle.trim().split(/\s+/)[0]) || "Ad";
  const personFamilyName = customerFamilyName || (customerTitle && customerTitle.trim().split(/\s+/).slice(1).join(" ")) || customerTitle || "Soyad";
  const customerStreet = customer.street || customer.adres || "";
  const customerCity = customer.city || "";
  const customerDistrict = customer.district || customer.ilce || "";
  const customerPostalZone = customer.postaKodu || customer.postalZone || "00000";
  const customerRegion = customer.region || customer.bolge || customer.city || customer.district || "Türkiye";
  const customerCountry = customer.country || "Türkiye";

  // AdditionalDocumentReference'ler (XSLT + CustInvID) – Taxten 2.1.6 / 2.1.7
  const additionalDocs = [];

  // 1. XSLT Şablonu (ZORUNLU – sendUBL ile tüm gönderimlerde; GİB görüntüleyici ve alıcı sistem için)
  // Dosya adı: FaturaID.xslt formatında (ör. IN02015000000001.xslt veya müşteri no + .xslt)
  const xsltFilename = (invoiceNumber && invoiceNumber !== "AUTO" ? invoiceNumber : (custInvId || "FaturaID")) + ".xslt";
  additionalDocs.push({
  ...
  // 2. CustInvID (Fatura ID platformda otomatik üretiliyorsa zorunlu; max 64 karakter; tekrar gönderimde aynı ID için aynı CustInvID)
  if (custInvId) {
    const custInvIdTrim = String(custInvId).slice(0, 64);
    additionalDocs.push({
      "cbc:ID": custInvIdTrim,
      "cbc:IssueDate": formattedDate,
  ...
  const doc = {
    "Invoice": {
      ...
      // UBL-TR XSD: LineCountNumeric ve currency kodları AdditionalDocumentReference'dan ÖNCE olmalı
      "cbc:LineCountNumeric": String(items.length),
      "cbc:TaxCurrencyCode": currency,
      "cbc:PricingCurrencyCode": currency,
      "cbc:PaymentCurrencyCode": currency,
      ...
      "cac:AccountingSupplierParty": {
        "cac:Party": {
          ...
          "cac:PostalAddress": {
            "cbc:PostalZone": supplierPostalZone,
            "cbc:Region": supplierRegion,
            "cbc:District": supplierDistrict,
            "cac:Country": {
              "cbc:Name": country,
              "cbc:IdentificationCode": { "@listID": "ISO3166-1", "@listAgencyName": "United Nations Economic Commission for Europe", "#text": country === "Türkiye" ? "TR" : "TR" }
            },
            "cbc:CityName": city || "-",
      ...
      "cac:AccountingCustomerParty": {
        "cac:Party": {
          ...
          ...(isCompany ? {} : {
            "cac:Person": {
              "cbc:FirstName": personFirstName,
              "cbc:FamilyName": personFamilyName
            }
          }),
          "cac:PostalAddress": {
            "cbc:PostalZone": customerPostalZone,
            "cbc:Region": customerRegion,
            "cbc:District": customerDistrict || customerCity || "-",
            "cac:Country": {
              "cbc:Name": customerCountry,
              "cbc:IdentificationCode": { "@listID": "ISO3166-1", "@listAgencyName": "United Nations Economic Commission for Europe", "#text": customerCountry === "Türkiye" ? "TR" : "TR" }
            },
