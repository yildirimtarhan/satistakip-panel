export async function sendMailApiBrevo({ to, subject, html, text, attachments }) {
  ...
  // ✅ PDF ve diğer ekler için destek (Base64 formatında)
  if (Array.isArray(attachments) && attachments.length > 0) {
    payload.attachment = attachments.map((a) => ({
      name: a.filename || a.name || "attachment.pdf",
      content: typeof a.content === "string" 
        ? a.content 
        : Buffer.from(a.content).toString("base64"),
      type: a.contentType || a.type || "application/pdf",
    }));
  }
...
// ✅ E-fatura PDF gönderimi için alias (attachments destekli)
export async function sendMail({ to, subject, html, text, attachments }) {
  return await sendMailApiBrevo({ to, subject, html, text, attachments });
}