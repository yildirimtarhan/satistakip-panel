...
import { getPttAvmCredentialsByCompany } from "@/lib/getPttAvmCredentials";
...
export async function pushStockToMarketplaces(productIds, ctx) {
  ...
  setImmediate(async () => {
    try {
      ...
      // ─── PTT AVM (multi-tenant: companyId ile settings'ten alır) ───
      try {
        const creds = await getPttAvmCredentialsByCompany(companyIdStr);
        if (creds?.apiKey && creds?.accessToken) {
          const items = [];
          for (const p of products) {
            const barcode = String(p.barcode || p.sku || "").trim();
            if (!barcode) continue;
            const quantity = Math.max(0, Number(p.stock ?? 0));
            const priceWithVAT = Number(p.priceTl ?? p.price ?? 0);
            items.push({
              barcode,
              quantity,
              ...(priceWithVAT > 0 ? { priceWithVAT, vatRate: 18 } : {}),
            });
          }
          if (items.length) {
            await pttAvmUpdateStockPrices(creds, items);
            console.log(`[pazaryeriStockSync] PTT AVM: ${items.length} ürün stok güncellendi`);
          }
        }
      } catch (e) {
        console.warn("[pazaryeriStockSync] PTT AVM:", e.message);
      }
    } catch (e) {
